...
import { setGridParams } from '../../store/gridActions'
...
function FormFirebaseContainer(props) {
	...
	const setGridInfo = (participants, tournament, tournamentId, categories, categoryId, grids) => {
		...
		dispatch(setGridParams({ participants, tournament, category, categoryId, tournamentId }))
		...
		if (grids && grids[categoryId]) {
			...
			if (gridType === 'group') {
				const fillGroupGrid = () => {
					...
					dispatch(setGridParams({ group1grid, group2grid }))
					dispatch(
						setGridParams({
							groupParticipants: [
				...
				dispatch(setGridParams({ grid: grids[categoryId]['grid'] }))
			}
		}
	}
